 ...
 * them against `.loopwright/baseline.json`, and decides whether the PR may merge.
 *
...
if (UPDATE_BASELINE) {
  ...
  if (failed.length > 0) {
    ...
    console.error('Fix the collector(s) and re-run `node .loopwright/scripts/run-report.mjs --all` first.');
    process.exit(2);
  }

  const next = {
    $comment:
      'Generated by `node .loopwright/scripts/quality-gate.mjs --update-baseline`. Commit this file — it is the ratchet the gate compares every PR against. Only regenerate deliberately, and say why in the PR.',
