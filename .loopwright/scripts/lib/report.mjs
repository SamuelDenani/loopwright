...
const REMEDIATION = {
  ...
  'audit.suppressed':
    'A suppression in `.loopwright/config.json` is holding this back. Re-check whether a fix has shipped and remove the entry.',
  'audit.':
    'Upgrade the offending dependency, or replace it. There is no prod/dev exemption — this PR has to be deployable. If there is genuinely no fix upstream, add an entry with an `expires` date to `audit.ignore` in `.loopwright/config.json` and say why in the PR.',
...
export function renderMarkdown(result) {
  ...
  if (result.verdict === 'pass' && groups.improvedMetrics.length > 0) {
    out.push(
      `> 📈 This PR improves ${groups.improvedMetrics.length} metric(s). Run \`node .loopwright/scripts/quality-gate.mjs --update-baseline\` and commit \`.loopwright/baseline.json\` to lock the gain in.`,
      '',
    );
  }
  ...
  out.push(
    '---',
    '<sub>Generated by `.loopwright/scripts/quality-gate.mjs` · reproduce locally with `node .loopwright/scripts/run-report.mjs --all && node .loopwright/scripts/quality-gate.mjs` · full reports are in the workflow artifacts.</sub>',
  );
